...
Основной сценарий: подписчик дает только IP сервера и пароль, агент сам подключается, диагностирует состояние, выбирает узкий режим работы и не трогает лишние подсистемы.
...
- Быстрая диагностика проблем у подписчиков.

## Быстрый Старт Для Подписчика
...
- `subscriber-intake.md` — что спрашивать у подписчика.
...
- Для подписчиков по умолчанию просить только IP и пароль сервера.
